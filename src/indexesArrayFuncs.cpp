...
void printSourceArrayData(StationReport r[], int size) {
    ...
    for (int idx = 0; idx < size; idx++) {
        std::cout << r[idx].reportIndex << " " << r[idx].stationNumber << " " << r[idx].firstName << " " << r[idx].lastName << " " << r[idx].rank << " " << r[idx].airTemperature << " " << r[idx].windSpeed << " " << r[idx].threatsEliminated << " " << r[idx].reportDate.tm_year + 1900 << "-" << r[idx].reportDate.tm_mon+1 << "-" << r[idx].reportDate.tm_mday << " " << r[idx].reportTimeSeconds << std::endl;
